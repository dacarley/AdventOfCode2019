...
updateGrid(l1, 1);
updateGrid(l2, 2);
grid[originX + (originY * w)] = 0;

const d1 = findDistances(l1);
const d2 = findDistances(l2);
const d = [];
for (let i = 0; i < d2.length; ++i) {
  for (let j = 0; j < d1.length; ++j) {
    if (d2[i].x === d1[j].x && d2[i].y === d1[j].y) {
      d.push(d2[i].distance + d1[j].distance);
    }
  }
}
...
function findDistances(list) {
  const distances = [];
  let distance = 0;
  const p = [originX, originY];
  
  for (let i = 0; i < list.length; ++i) {
    const item = list[i];
    const dir = item[0];
    let amount = parseInt(item.slice(1));
    while (amount > 0) {
      switch (dir) {
        case "U":
          p[1] -= 1;
